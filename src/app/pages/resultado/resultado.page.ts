...
export class ResultadoPage implements OnInit {
  ...
  toPostfix() {
    ...
    for (const caracter of this.infija) {
      if (!this.operadores.includes(caracter)) {
        ...
        while (
          opStack.length != 0 &&
          prec[opStack[opStack.length - 1]] > prec[caracter]
        ) {
  ...
  generarTabla() {
    ...
    for (const caracter of this.postfija) {
      if (
        !this.operadores.includes(caracter) &&
        !this.variables.includes(caracter)
      ) {
  ...
  sustituir(combinacion, postfija) {
    ...
    for (const caracter of auxPost) {
      if (this.variables.includes(caracter)) {
        auxPost = this.replaceAll(
          auxPost,
          caracter,
          combinacion[this.variables.indexOf(caracter)]
        );
      }
    }
